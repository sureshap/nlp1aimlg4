...
class InputTransformer(BaseEstimator, TransformerMixin):
    ...
    def preprocess_input(input_text):
        ...
        process_text = input_text.apply(InputTransformer.remove_whitespace_CR)
        #print(type(process_text))
        print("Inside process input -after remove_whitespace_CR",process_text )
        process_text = process_text.apply(InputTransformer.remove_accents)
        print("Inside process input -after remove_accents",process_text )    
        process_text = process_text.apply(InputTransformer.Translate_text)
        print("Inside process input -after Translate_text",process_text )
        process_text = process_text.apply(InputTransformer.preprocess_text)
        print("Inside process input -after Preprocessing",process_text )   
        process_text = process_text.apply(InputTransformer.preprocess_vocab)
    ...
    def transform(self, X, y = None):
        ...
        X = InputTransformer.preprocess_input(X)
        return X
